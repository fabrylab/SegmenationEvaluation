...
class UNet_filters():
    ...
    def create_model(self, img_shape, num_class, d=2):
        ...
        conv1 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(conv1)
        pool1 = layers.MaxPooling2D(pool_size=(2, 2))(conv1)
        conv2 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(pool1)
        conv2 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(conv2)
        pool2 = layers.MaxPooling2D(pool_size=(2, 2))(conv2)

        conv3 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(pool2)
        conv3 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(conv3)
        pool3 = layers.MaxPooling2D(pool_size=(2, 2))(conv3)

        conv4 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(pool3)
        conv4 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(conv4)
        pool4 = layers.MaxPooling2D(pool_size=(2, 2))(conv4)

        conv5 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(pool4)
        conv5 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(conv5)

        up_conv5 = layers.UpSampling2D(size=(2, 2))(conv5)
        ch, cw = self.get_crop_shape(conv4, up_conv5)
        crop_conv4 = layers.Cropping2D(cropping=(ch,cw))(conv4)
        up6 = layers.concatenate([up_conv5, crop_conv4], axis=concat_axis)
        conv6 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(up6)
        conv6 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(conv6)

        up_conv6 = layers.UpSampling2D(size=(2, 2))(conv6)
        ch, cw = self.get_crop_shape(conv3, up_conv6)
        crop_conv3 = layers.Cropping2D(cropping=(ch,cw))(conv3)
        up7 = layers.concatenate([up_conv6, crop_conv3], axis=concat_axis)
        conv7 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(up7)
        conv7 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(conv7)

        up_conv7 = layers.UpSampling2D(size=(2, 2))(conv7)
        ch, cw = self.get_crop_shape(conv2, up_conv7)
        crop_conv2 = layers.Cropping2D(cropping=(ch,cw))(conv2)
        up8 = layers.concatenate([up_conv7, crop_conv2], axis=concat_axis)
        conv8 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(up8)
        conv8 = layers.Conv2D(d, (3, 3), activation='relu', padding='same')(conv8)
